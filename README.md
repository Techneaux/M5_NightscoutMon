...
There are currently 4 display pages, that can be changed by a short press of the right button. The button does not work when the device communicated with Nightscout (small blue WiFi icon is displayed).
...
**Page 3 - simplified glucose display with full-width graph**
Optimized for readability with a large centered glucose value, trend arrow, and delta value. Features a full-width 2-hour trend graph (24 readings) with range 40-400 mg/dL. A red banner appears when data is more than 10 minutes stale.
...
**Building from source:** Clone the repository and build using PlatformIO (see "Building with PlatformIO" section below). The source code is in the `pio_project/` directory and all dependencies are automatically downloaded during the first build.

**Pre-built firmware:** Download the latest [M5Burner release](https://github.com/mlukasek/M5_NightscoutMon/releases). It is a Windows executable with binary M5Stack firmware included. Just unzip it, start the M5Burner.exe, choose COM port where your M5Stack is connected and burn the firmware. Remember to add your microSD card with M5NS.INI configuration file. See the [wiki](https://github.com/mlukasek/M5_NightscoutMon/wiki) for details.
...
The source code is located in the `pio_project/` directory. You can build using either VSCode with PlatformIO extension or the PlatformIO CLI.

#### Option 1: VSCode with PlatformIO Extension
...
* Open the project: `File -> Open Folder...` and select the `pio_project/` directory
* To **build**: Click the checkmark icon on the blue bottom toolbar (or press `Ctrl+Alt+B`)
* To **upload**: Connect your M5Stack via USB-C, then click the arrow icon on the blue bottom toolbar (or press `Ctrl+Alt+U`)
...
The first build will download the ESP32 toolchain and M5Stack libraries (~500MB). This is automatic and only happens once.
